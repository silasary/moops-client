...
{---------------------------------------------------------------------------
 Date last modified: 1999-11-11
{---------------------------------------------------------------------------

{---------------------------------------------------------------------------
 C++ Language Syntax Parser v0.78
{---------------------------------------------------------------------------
 Revision History:
...
uses
  ...
  mwHighlighter, mwLocalStr, {gp 12/16/1998}
  mwExport;
...
procedure Register;
begin
  RegisterComponents(MWS_HighlightersPage, [TDcjCppSyn]);
end;
...
function TDcjCppSyn.KeyComp(const aKey: String): Boolean;
var
  I: Integer;
  Temp: PChar;
begin
...
begin
  fAsmAttri := TmwHighLightAttributes.Create(MWS_AttrAssembler);
  ...
  fInvalidAttri := TmwHighLightAttributes.Create(MWS_AttrIllegalChar);
  ...
  fRange := rsUnknown;
  fAsmStart := False;
  fDefaultFilter := MWS_FilterCPP;
end; { Create }

procedure TDcjCppSyn.SetLine(NewValue: String; LineNumber:Integer);
begin
...
procedure TDcjCppSyn.QuestionProc;
begin
...
function TDcjCppSyn.GetExtTokenID: TxtkTokenKind;
begin
  Result := FExtTokenID;
end;

function TDcjCppSyn.GetTokenAttribute: TmwHighLightAttributes;
begin
  case fTokenID of
...
function TDcjCppSyn.GetTokenKind: integer;
begin
...
//          were preserved
  ...
  var
    ...
    tmpInvalidAttri   : TmwHighLightAttributes;
  ...
  begin { ReadCPPBSettings }
    ...
    try
      ...
      if settingIndex >= s.Count then Result := false
        ...
        tmpInvalidAttri   := TmwHighLightAttributes.Create('');
...
function TDcjCppSyn.GetLanguageName: string;
begin
  Result := MWS_LangCPP;
end;

function TDcjCppSyn.GetCapability: THighlighterCapability;
begin
  Result := inherited GetCapability + [hcUserSettings,hcExportable];
end;

procedure TDcjCppSyn.SetLineForExport(NewValue: String);
begin
  fLine := PChar(NewValue);
  Run := 0;
  ExportNext;
...
procedure TDcjCppSyn.ExportNext;
begin
  fAsmStart := False;
  fTokenPos := Run;
  Case fRange of
  ...
  if Assigned(Exporter) then
    with TmwCustomExport(Exporter) do begin
      Case GetTokenID of
        ...
        tkDirective:FormatToken(GetToken, fDirecAttri, True,False);
