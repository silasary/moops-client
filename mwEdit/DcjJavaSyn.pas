...
{---------------------------------------------------------------------------
 Java Language Syntax Parser v0.57
{---------------------------------------------------------------------------
 Revision History:
...
procedure Register;
begin
  RegisterComponents(MWS_HighlightersPage, [TDcjJavaSyn]);
end;
...
function TDcjJavaSyn.KeyComp(const aKey: String): Boolean;
var
  I: Integer;
  Temp: PChar;
begin
...
begin
  ...
  InitIdent;
  MakeMethodTables;
  fDefaultFilter := MWS_FilterJava;
end; { Create }

procedure TDcjJavaSyn.SetLine(NewValue: String; LineNumber:Integer);
begin
...
function TDcjJavaSyn.GetTokenAttribute: TmwHighLightAttributes;
begin
  case fTokenID of
...
function TDcjJavaSyn.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;
